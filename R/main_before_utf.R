...
irsa.default <- function(finess, annee, mois, path, lib = T, typi = 4, tolower_names = F, ...){
  ...
  format$fin <- cumsum(format$longueur)
  libelles <- format %>% dplyr::pull(libelle)
  ...
  if (typi !=0){
    ...
    rsa_i <- vroom::vroom_fwf(paste0(path,"/",finess,".",annee,".",mois,".rsa"),
                              col_positions = vroom::fwf_positions(start =     format %>% dplyr::pull(position),
                                                                   end =       format %>% dplyr::pull(fin),
                                                                   col_names = format %>% dplyr::pull(nom)),
                              col_types = format %>% dplyr::pull(type) %c% '',na=character(), ...)
...
itra.default <- function(finess, annee, mois, path, lib = T, champ= "mco", tolower_names = F, ... ){
  ...
  format <- format %>% dplyr::mutate(type = ifelse(nom %in% c('DTENT', 'DTSORT'), 'D', type))
  format$fin <- cumsum(format$longueur)
  if (champ=="mco"){

    tra_i <- vroom::vroom_fwf(paste0(path,"/",finess,".",annee,".",mois,".tra.txt"),
                          col_positions = vroom::fwf_positions(start =     format %>% dplyr::pull(position),
                                                               end =       format %>% dplyr::pull(fin),
                                                               col_names = format %>% dplyr::pull(nom)),
                          col_types = format %>% dplyr::pull(type) %c% '',
  ...
  if (lib==T & champ !="psy_r3a"){
    ...
    v <- c(format %>% pull(libelle), 'Établissement')
...
iano_mco.default <- function(finess, annee, mois, path, typano = c("out", "in"), lib = T, tolower_names = F, ...){
  ...
  if (typano=="out"){
    ...
    libelles <- format %>% dplyr::pull(libelle)
    if (annee>=2013){
      ano_i <- vroom::vroom_fwf(paste0(path,"/",finess,".",annee,".",mois,".ano"),
                            col_positions = vroom::fwf_positions(start =     format %>% dplyr::pull(position),
                                                                 end =       format %>% dplyr::pull(fin),
                                                                 col_names = format %>% dplyr::pull(nom)),
                            col_types = format %>% dplyr::pull(type) %c% '',
                            locale = readr::locale(date_format = "%d%m%Y"),na=character(), ...)
